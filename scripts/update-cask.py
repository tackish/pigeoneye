...
cask path). The cask lives in the separate tap repo (tackish/
homebrew-pigeoneye), which the workflow checks out, so the path is
passed in. The first sha256 in the file belongs to the arm build, the
second to intel — the same order they appear in the cask.
"""
import re
import sys

version, arm, amd, path = sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4]
src = open(path).read()
