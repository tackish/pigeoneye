...
// Issues transition: with a Pod list loaded (selected() set), opening the
// cross-cluster Issues view flips selected()->null + customView="issues" in
// the same tick. The content subtree must tear down without a nested
// selected()!.<x> re-evaluating on null (regression: "null is not an object
// (evaluating selected().deletable)" / .kind).
{
  // close the New modal first (Escape)
  root.querySelector(".new-modal") && document.body.dispatchEvent(new dom.window.KeyboardEvent("keydown", { key: "Escape", bubbles: true }));
  await new Promise((r) => setTimeout(r, 100));
  const issuesBtn = [...root.querySelectorAll("button.view-kind")].find((b) => b.textContent?.includes("Issues"));
  if (!issuesBtn) fail("no Issues sidebar button", root.innerHTML.replace(/<[^>]+>/g, "").slice(0, 300));
  issuesBtn.click();
  await new Promise((r) => setTimeout(r, 300));
  if (crash) fail("crash on opening Issues while a list is loaded (selected()->null transition)", crash);
  if (!root.querySelector(".iss-body") && !root.querySelector(".iss-scan"))
    fail("Issues view did not render after click", root.innerHTML.replace(/<[^>]+>/g, "").slice(0, 300));
}
